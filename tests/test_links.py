...
class TestLinks:
    def test_broken_links(self, setup, logger):
        ...
        with open("broken_links.log", "w", encoding="utf-8") as broken_links_log, \
            ...
            session = requests.Session()  # Maintain session to handle cookies
            ...
            for link in all_links:
                ...
                try:
                    response = session.get(
                        ...
                        headers={
                            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
                            "Referer": base_url,  # Helps avoid 403 errors by mimicking navigation from your site
                    ...
                    status_code = response.status_code  # ✅ Correctly use actual HTTP response status

                except requests.RequestException as e:
                    logging.error(f"❌ Request failed for {full_link}: {str(e)}")
                    status_code = 500  # Assign 500 when the request fails

                # Ensure status_code is always set
                ...
                # ✅ Now correctly checking if the link is broken
                if status_code == 403:
...
#                     page_ready_state = browser.execute_script("return document.readyState")
#                     logging.info(f"Page load state: {page_ready_state}")
#                     status_code = 200 if page_ready_state == 'complete' else 500
#
#                     response = session.get(full_link, allow_redirects=True, timeout=5)
#                     # status_code = None
...
#                     if status_code >= 400:
#                         logging.warning(
#                             f"⚠️ Broken Link: {full_link} → Status {status_code} | Found in: {element_text} | Page: {source_page}")
#                         broken_links_log.write(
#                             f"{timestamp} | {full_link} → Status {status_code} | Found in: {element_text} | Page: {source_page}\n")
#                         print(f"❌ Broken link detected: {full_link} | Page: {source_page} | Element: {element_text}")
#                         broken_count += 1
#                     else:
#                         logging.info(f"✅ Working Link: {full_link} → Status {status_code}")
#                         working_links_log.write(f"{timestamp} | {full_link} → Status {status_code}\n")
...
#                     print(
#                         f"❌ Exception caught for: {full_link} - {str(e)} | Page: {source_page}")
#
#         # Print Summary:
#         print("\n📊 Test Summary:")
#         print(f"🔗 Total links found: {len(all_links)}")
